...
RUN apt-get install -y --no-install-recommends --allow-unauthenticated supervisor openssh-server pwgen sudo vim-common net-tools \
        ...
        nginx python-dev build-essential mesa-utils libgl1-mesa-dri \
...
RUN wget https://downloads.slack-edge.com/linux_releases/slack-desktop-4.16.0-amd64.deb
RUN dpkg -i slack-desktop-4.16.0-amd64.deb; apt-get -fy install
